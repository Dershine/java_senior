...
* 服务雪崩如何解决
  * 服务雪崩
    * 一个服务失败，导致整条链路的服务都失败
  * 服务降级
    * 服务自我保护的一种方式，确保服务不会收到突增的请求影响变得不可用，一般在实际开发中与feign接口整合，编写降级逻辑
  * 服务熔断
...
* 微服务限流
  * 场景
    * 抢购优惠券，为了应对突发流量，需要做限流
    * 常规限流，为了防止恶意攻击

  * 限流方案
    * nginx限流
      * 控制速率（突发流量），使用漏桶算法实现过滤，让请求以固定速率处理，可应对突发流量
...
* 分布式事务解决方案
  * seata的XA模式，CP需要互相等待格个分支事务提交，可以保证强一致性，性能差  ***银行业务***
